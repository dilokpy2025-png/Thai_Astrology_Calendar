...
const state = (() => { const now = new Date();
...
function pos(x) { 
				x=mod(x); 
				const r=x/RASI, rem=x%RASI; 
				return {rasi:Number(r), degree:Number(rem/60n), lipda:Number(rem%60n)}; 
				}
				
function arc(angle) { 
					angle=mod(angle); 
					if(angle<=5400n)
						return [angle,-1n,5400n-angle,1n];
					if(angle<=10800n)
						return [10800n-angle,-1n,angle-5400n,-1n]; 
					if(angle<=16200n)
						return [angle-10800n,1n,16200n-angle,-1n]; 
					return [21600n-angle,1n,angle-16200n,1n]; 
					}
					
function phuj(x,t) { 
					const i=Number(x/RASI), rem=x%RASI, base=t[i], next=t[i+1]??base;
					return ((rem*(next-base)+base*RASI)*60n)/RASI; 
					}
function ko(x,t) { 
					const i=Number(x/RASI), rem=x%RASI, base=t[i], next=t[i+1]??base; 
					return (rem*(next-base)+base*RASI+900n)/RASI; 
					}
function sun(h,minutes) { 
					const raw=h*800n+part(minutes,800n)-373n, meanMotion=raw%292207n; 
					let [r,rem1]=[meanMotion/24350n,meanMotion%24350n], [o,rem2]=[rem1/811n,rem1%811n], l=rem2/14n-3n; if(l<0n)l+=60n; 
					const mean=r*RASI+o*60n+l, [p,sign]=arc(mean-4800n), table=[0n,35n,67n,94n,116n,129n,134n], i=Number(p/900n), correction=table[i]+(p%900n)*(table[i+1]-table[i])/900n; 
					return {...pos(mean+correction*sign),mean,kam:raw%292207n}; 
					}
function moon(h,meanSun,minutes) { 
					const mas=h*703n+650n+part(minutes,703n), 
					      score=mas%20760n, 
						  dithi=score/692n, 
						  avaman=score%692n, 
						  mean=mod(dithi*720n+(104n*avaman)/100n-40n+meanSun), 
						  uch=((((h-621n)%3232n)*DAY_MINUTES+BigInt(minutes))*CIRCLE)/(3232n*DAY_MINUTES)+2n, 
						  [p,sign]=arc(mean-uch), 
						  table=[0n,77n,148n,209n,256n,286n,296n], 
						  i=Number(p/900n), 
						  correction=table[i]+(p%900n)*(table[i+1]-table[i])/900n; 
						  return pos(mean+correction*sign); 
					}
function solve(mean,firstRef,monBase,secondRef,baseChet,payatNum,payatDen,roundSing=false) { 
					const table=[0n,244n,427n,488n]; 
					let [p,sign,k,signK]=arc(firstRef), 
						mchet=baseChet+(ko(k,table)/2n)*signK, 
						mon=mod(monBase+(phuj(p,table)*60n/mchet)*sign); 
					let [p2,sign2,k2,signK2]=arc(mon-secondRef), 
						sp=phuj(p2,table), 
						sing=roundSing?((sp+30n)/60n)/3n:(2n*(sp/60n)+1n)/6n, 
						schet=sing+(payatNum===null?payatDen:(mchet*payatNum)/payatDen)+ko(k2,table)*signK2; 
						return pos(mon+(sp*60n/schet)*sign2); 
					}
function calculate(year,month,day,hour,minute) { 
					const g=year-543, target=Date.UTC(g,month-1,day); 
					if(new Date(target).getUTCDate()!==day)throw new Error("วันที่ไม่ถูกต้อง"); 
					let cs=BigInt(year-1181), song=Date.UTC(g,3,16); 
					if(target<song){song=Date.UTC(g-1,3,16);cs-=1n;} 
					const sur=BigInt(Math.floor((target-song)/86400000)), 
						  seed=292207n*cs+373n, 
						  hor=seed/800n+sur, 
						  mins=hour*60+minute, 
						  s=sun(hor,mins), 
						  m=moon(hor,s.mean,mins), 
						  ravi=mod(s.mean-23n), 
						  kam=(cs-(s.kam<364n?611n:610n))*CIRCLE+ravi;
					const marsMean=mod(kam/2n+(kam*16n)/505n+5420n), 
						  mercuryMean=mod((kam*7n)/46n+kam*4n+10642n), 
						  jupiterMean=mod(kam/12n+kam/1032n+14297n), 
						  venusMean=mod((kam*5n)/3n-(kam*10n)/243n+10944n), 
						  saturnMean=mod(kam/30n+(kam*6n)/10000n+11944n), 
						  uranusMean=mod(kam/84n+kam/7224n+16277n);
					const rahu=pos(15150n-mod(kam/20n+kam/265n)), 
						  ketu=pos(CIRCLE-((((hor-344n)%679n)*DAY_MINUTES+BigInt(mins))*CIRCLE)/(679n*DAY_MINUTES));
				return {cs,hor, planets:{"อาทิตย์":s,
										 "จันทร์":m,
										 "อังคาร":solve(marsMean,marsMean-7620n,marsMean,ravi,2700n,4n,15n),
										 "พุธ":solve(mercuryMean,ravi-13200n,ravi,mercuryMean,6000n,null,1260n,true),
										 "พฤหัส":solve(jupiterMean,jupiterMean-10320n,jupiterMean,ravi,5520n,3n,7n),
										 "ศุกร์":solve(venusMean,ravi-4800n,ravi,venusMean,19200n,null,660n),
										 "เสาร์":solve(saturnMean,saturnMean-14820n,saturnMean,ravi,3780n,7n,6n),
										 "ราหู":rahu,
										 "เกตุ":ketu,
										 "มฤตยู":solve(uranusMean,uranusMean-7440n,uranusMean,ravi,38640n,3n,7n)}}; }

function pad(n){return String(n).padStart(2,"0");}
function renderCalendar(){ 
			const cal=$("calendar"); cal.textContent=""; 
			const g=state.year-543, 
				  first=new Date(Date.UTC(g,state.month-1,1)).getUTCDay(), 
				  days=new Date(Date.UTC(g,state.month,0)).getUTCDate(), 
				  today=new Date(); 
			...
			for(let d=1;d<=days;d++){
				const b=document.createElement("button");
					  b.className="day"+(d===state.day?" selected":"")+(today.getFullYear()===g&&today.getMonth()+1===state.month&&today.getDate()===d?" is-today":"");
					  b.textContent=d;b.onclick=()=>{state.day=d;render();};cal.append(b);} 
		}
...
function initTimePicker() {
  ...
  for (let m = 0; m < 60; m++) {
    minute.add(new Option(pad(m), String(m)));
  }
...
function renderPlanetTable(planets) {
  ...
  PLANETS.forEach(([name, symbol, color], index) => {
    ...
    numberCell.style.setProperty("--planet-color", color);
...
initTimePicker();
$("hour").addEventListener("change", renderResults);
$("minute").addEventListener("change", renderResults);
$("today").onclick = () => {
  const n = new Date();
  Object.assign(state, {
